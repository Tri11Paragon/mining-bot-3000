...
public class BlockUtils {
    ...
    public static MobSpawn isValidMobSpawn(BlockPos blockPos, boolean newMobSpawnLightLevel, boolean ignoreLightLevel){
        return isValidMobSpawn(blockPos, mc.world.getBlockState(blockPos), newMobSpawnLightLevel ? 0 : 7, ignoreLightLevel);
    }

    public static MobSpawn isValidMobSpawn(BlockPos blockPos, BlockState blockState, int spawnLightLimit) {
        return isValidMobSpawn(blockPos, blockState, spawnLightLimit, false);
    }

    public static MobSpawn isValidMobSpawn(BlockPos blockPos, BlockState blockState, int spawnLightLimit, boolean ignoreLightLevel) {
        ...
        if (!topSurface(downState)) {
            if (downState.getCollisionShape(mc.world, down) != VoxelShapes.fullCube() && mc.world.getBlockState(blockPos.down()).getBlock() != Blocks.SOUL_SAND)
